...
def resolve_host_name(host_name_to):
  ...
  result = {'host_name': None, 'ip_address': None,'ttl':None}
  ...
  if (response_code == "0"):
    ...
    # adun ttl la data curenta ca sa stiu cand va expira
    currDate = datetime.datetime.now()
    if result['ttl'] is not None:
      currDate = currDate + datetime.timedelta(seconds=result['expire'])
      result['ttl'] = currDate
...
def DNSCache(HOST_NAME,adresses):
  if (len(adresses) == 0):
    adresses = resolve_host_name(HOST_NAME)
    if adresses is None:
      return None

  else:
    if adresses['ttl'] is not None:
      if adresses['ttl'] < datetime.datetime.now():
        adresses = resolve_host_name(HOST_NAME)
